...
test('a server that needs auth shows the sign-in steps, and its button opens the Terminal running claude', async ({ page }) => {
  ...
  await expect(page.getByTestId('conversation-terminal-title')).toBeVisible()
  await page.getByTestId('open-flow').click()
  await page.getByTestId('flow-new').click()
  await page.getByTestId('flow-source-ado').click()
  await page.getByTestId('flow-ado-state').getByTestId('flow-ado-terminal').click()
  await expect(page.getByTestId('flow-popup')).toHaveCount(0)
  await expect(page.getByTestId('terminal-pane')).toBeVisible()
  await page.getByTestId('open-flow').click()
  await page.getByTestId('flow-new').click()
...
test('a sign-in that ends while Flow is closed on another project no longer shows as waiting when Flow opens again', async ({ page }) => {
  await openFlow(page)
  await page.evaluate(() => window.__mock.setAdoConnected(false, 'it failed to start: spawn npx ENOENT'))
  await page.getByTestId('flow-new').click()
  await page.getByTestId('flow-source-ado').click()
  ...
  await page.getByTestId('flow-ado-reconnect').click()
  await expect(page.getByTestId('flow-ado-signing-in')).toBeVisible()
  ...
  await page.getByTestId('flow-new').click()
  await page.getByTestId('flow-source-ado').click()
  await expect(page.getByTestId('flow-features-progress')).toBeVisible()
