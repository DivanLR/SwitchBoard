...
describe('buildTranscript', () => {
  ...
  it('names no path to a container session, which cannot read the host temp folder', () => {
    const built = buildTranscript(session(), 'alpha', [event('prompt', { text: 'carry me' })], SAVED_AT)
    const append = transcriptContextAppend({ ...built.summary, path: '/tmp/t/sess-1.md' }, false)
    expect(append).toContain(built.summary.digest)
    expect(append).not.toContain('/tmp/t/sess-1.md')
  })
})
...
describe('the temp directory', () => {
  ...
  it('writes a transcript and reads its own header back', () => {
    ...
    const listed = transcriptFor('sess-1', SAVED_AT)
  ...
  it('offers nothing once every transcript has expired', () => {
    ...
    expect(transcriptFor('sess-1')).toBeNull()
