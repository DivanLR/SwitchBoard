...
describe.runIf(enabled)('real Claude Code session (quickstart smoke)', () => {
  it('streams events, intercepts a permission, delivers the approval, and completes the turn', async () => {
    ...
    await vi.waitFor(() => expect(repos.sessions.byId(session.id)?.endedAt).not.toBeNull(), {
      timeout: 30_000,
      interval: 500,
    })
    db.close()
...
describe.runIf(enabled)('real AskUserQuestion routing (T021 watch item)', () => {
  it('renders a question as a stream event and delivers the clicked answer', async () => {
...
describe.runIf(enabled)('a section dispatching to a session it just started', () => {
  it('keeps the background session alive and gets its answer', async () => {
