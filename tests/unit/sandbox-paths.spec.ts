...
describe('browser in the bypass container, only where earned', () => {
  ...
  it('is present for an Angular workspace, whose unit tests run ChromeHeadless', () => {
    ...
    for (const id of ['ng-unit', 'ng-coverage', 'ng-e2e']) {
  ...
  it('gives a .NET project with an Angular front end both toolchains and the browser', () => {
    const tools = sandboxTools(true, true)
    expect(tools).toEqual(['node', 'dotnet', 'browser'])
    for (const id of ['dotnet-unit', 'ng-unit']) {
      expect(unavailableReason(suiteById(id)!, tools)).toBeNull()
    }
    expect(unavailableReason(suiteById('ng-unit')!, sandboxTools(true, false))).toContain(
      'not in the bypass container',
    )
  })
})
