...
- The lane bar reports STATE, not identity, since 2026-08-12: green (`--running`) where a project has a live session, orange (`--idle`) where it has none. 2px, on the lane's RIGHT edge, the full height of the lane, full strength on every lane rather than dimmed on the ones you are not in, because "is this running" is true whether or not you are looking at the row. The per-project identity hue is gone from the lane and survives only on the group swatches. The reversal was asked for; the argument it overturns is kept in the code comment on `.brace`, because it is the reason six hashed hues were the wrong thing to put there and two named states are the right one.
...
**The accents came back into line.** The action colour was cornflower blue `#2B58CC` here for exactly one release, and is green again at `#136F45` (5.73:1 as text, 6.20:1 carrying white) on the owner's direction. What survives from the reference is its NEUTRALS: the cool canvas, the white cards, the soft hairlines, the 6px corners. Only the accent returned. Attention owed stays at `#8F5600`, where it landed while the action colour held blue. That is not a bright gold and cannot be: no saturated warm hue clears 4.5:1 on a near-white ground in sRGB, so amber here means the darkest legible amber. Error is `#A0141B`, pushed darker than a straight transposition so it separates from amber by luminance (1.34:1) as well as by hue.
...
- **Don't** give the lane bar a background, a gradient or a second colour. That is all that survives of this rule. The width cap and the ban on placing the bar at the row's outer edge were both overruled on request (1px to 2px, 2px to 4px on the left, then 2px on the RIGHT edge, where it sits directly inside the sidebar's own border). What the original rule was defending is a matter of record in the comment on `.brace`, not a constraint any longer.
