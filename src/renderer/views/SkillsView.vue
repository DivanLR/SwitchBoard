...
const sections = computed(() => {
  const favourites = favouriteNames.value
  ...
  for (const skill of skills.enabled) {
    ...
    groups.set(skill.sourceUrl, [...(groups.get(skill.sourceUrl) ?? []), skill])
  }
  return [
    ...(favourites.length > 0
      ? [{ key: 'favourites', label: 'Favourites', favourite: true, items: favourites }]
      : []),
...
<template>
  <div class="ui-body skills" data-testid="skills-view">
    <div class="ui-head">
      <div class="ui-meaning" :title="`Each skill runs in the Skills section's own session for ${projectName}.`">
        Imported skills, run against <span class="proj">{{ projectName }}</span> in their own session.
        Starred ones sit at the top.
      </div>
    ...
    <div v-if="skills.items.length === 0" class="ui-empty" data-testid="skills-empty">
      ...
      <div class="ui-empty-sub">Import a GitHub skills folder in Settings, Skills, and they appear here.</div>
    </div>
    ...
    >
      <div class="group-head">
        ...
        <span class="ui-chip">{{ group.items.length }} skill{{ group.items.length === 1 ? '' : 's' }}</span>
      </div>
      ...
      <div class="cmd-list">
        <div v-for="skill in group.items" :key="skill.name" class="cmd-wrap">
          <button
            class="cmd-fav"
            :class="{ on: favouriteSet.has(skill.name) }"
            :data-testid="`skill-fav-${skill.name}`"
            :aria-pressed="favouriteSet.has(skill.name)"
            :title="favouriteSet.has(skill.name) ? `Unpin ${skill.name} from the top` : `Pin ${skill.name} to the top`"
          ...
          >
            ...
            <span class="cmd-run">{{ running === skill.name ? 'Sending…' : 'Run' }}</span>
          </button>
          <button
            class="cmd-arg"
